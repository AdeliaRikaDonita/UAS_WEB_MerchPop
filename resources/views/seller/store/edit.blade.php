...
<div class="max-w-lg mx-auto bg-white rounded-2xl border border-plum-100 p-8">
    <h1 class="text-xl font-bold text-plum-900 mb-6">Edit Profil Toko</h1>
    ...
    <form action="{{ route('seller.store.update') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-lg border-plum-200 border px-3 py-2">{{ old('description', $store->description) }}</textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-blush-700 mb-1">Alamat</label>
            <textarea name="address" rows="2" class="w-full rounded-lg border-plum-200 border px-3 py-2">{{ old('address', $store->address) }}</textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-blush-700 mb-1">No. HP Toko</label>
            <input type="text" name="phone" value="{{ old('phone', $store->phone) }}" class="w-full rounded-lg border-plum-200 border px-3 py-2">
        </div>
